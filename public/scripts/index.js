...
function displayItem(item) {
  ...
  button.addEventListener('click', () => {
    window.location.href = "/shop?item=" + encodeURIComponent(item.id);
  });
}
...
if (blogsFeat) {
  fetch('/api/blogs')
    ...
    .then(data => {
      ...
      data.slice(0, 4).forEach(dataItem => {
        ...
        body.textContent = dataItem.body;
        timeofpost.textContent = new Date(dataItem.createdAt).toLocaleDateString();
